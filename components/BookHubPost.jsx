...
const BookHubPost = ({
  posts: {$id, content, bookImage, like, commentCount, userName, userImage,shareCount },
}) => {
  ...
  const submitComment = async () => {
    ...
    try {
      ...
      const createdComment = await createPostComment($id,user.username, newComment,updatedCommentCounts);  
  ...
  return (
    <>
      ...
      <View className="flex flex-row items-center pl-1.5 mb-0">
        ...
        />
        <Text className="ml-2 mb-3 font-semibold text-base leading-none">{userName}</Text>
      </View>
      ...
      <View className="flex flex-row mt-0 pt-0">
        ...
        <Text className="font-semibold text-base leading-none flex-wrap">{content}</Text>
        </View>
      </View>
