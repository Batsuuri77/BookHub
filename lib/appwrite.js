...
export async function getCurrentUser() {
  try {
    ...
    if (!currentUser) throw new Error("User data is not found.");
...
export async function getLatestBooks(userId) {
  if (!userId) {
    throw new Error("User must be logged in is required");
  }
  try {
    ...
    throw new Error(
      error.message || "Failed to fetch latest books. For this user"
    );
  }
}
...
export async function getAllBooksByUserId(userId) {
  if (!userId) {
    throw new Error("User must be logged in is required");
  }
  try {
...
export async function getAppwriteUserId() {
  try {
    const currentUser = await account.get();
    //console.log("Authenticated user fetched from Appwrite:", currentUser);
    return currentUser.$id;
  } catch (error) {
    console.error("Failed to fetch Appwrite authenticated user:", error);
    throw new Error("User is not authenticated.");
  }
}

//Get all notes by bookId
export async function getAllNotes(bookId, userId) {
  try {
    console.log("Fetching notes for bookId:", bookId, "and userId:", userId);

    const notes = await databases.listDocuments(
      appwriteConfig.databaseId,
      appwriteConfig.noteCollectionId,
      [Query.equal("bookId", bookId), Query.equal("userId", userId)]
    );
    ...
    // }));
    console.log("Notes fetched successfully:", notes.documents);
...
export async function createNote(noteData) {
  try {
    ...
    const appwriteUserId = await getAppwriteUserId();
    console.log("Authenticated User ID:", appwriteUserId);

    if (!appwriteUserId) {
      throw new Error("User is not authenticated. Cannot create a note.");
    }

    console.log("Setting permissions for user:", appwriteUserId);
